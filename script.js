  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      if (session) {
        ...
        saveSessions(); 
        renderChatWindow();
        alert('Chat history is cleared.');
      }
    } else {
      alert('Error in clearing chat history: ' + data.message);
    }
  } catch (error) {
    console.error('Error in clearing chat history:', error);
...
function deleteChatSession() {
  ...
  const confirmDelete = confirm('Do you want to delete this chat session?');
